...
<!-- <div class="row">
        ...
        <select id="standard" name="Smdetail[product_name]" class="custom-select rony1" required="required">
            <option value="">Select or Search Product</option>
            <?php
           //ysql_connect('localhost','root','');
           //ysql_select_db('ur2');
           //query = mysql_query("SELECT * FROM `cm_productmaster`");
           //hile($row = mysql_fetch_array($query)){
             // echo'
                
               //option value="'.$row['cm_code'].'">'.$row['cm_name'].'</option>
               //;
           //
           //>
       ///select> 
       //?php //ho $form->error($model,'cm_code'); ?>
    </div> -->
    <div  class="row">
        <label> Product Name </label>
        <?php $this->widget('zii.widgets.jui.CJuiAutoComplete', array(
            ...
            'options'=>array(
                ...
                'select'=>'js:function( event, ui ) {
                                        $("#product-name").text(ui.item.label);
