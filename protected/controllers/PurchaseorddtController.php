...
class PurchaseorddtController extends Controller
{
            ...
            if($model->validate())
            {
                ...
                if($result == 1){
                    ...
                    if($model->save()){
                        ...
                        $query = mysql_query("SELECT * FROM `pp_purchaseordhd` WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
                        //$command=$connection->createCommand($query);
                        //$command->execute();
                        $row = mysql_fetch_array($query);
                        $pp_discrate = $row['pp_discrate'];
                        $pp_amount = $row['pp_amount'];
                        $pp_discamt = $pp_amount * $pp_discrate / 100;
                        //echo $pp_discamt;
                        $pp_netamt = $pp_amount - $pp_discamt;
                        mysql_query("UPDATE pp_purchaseordhd SET pp_discamt='$pp_discamt',pp_netamt='$pp_netamt' WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
                    }else{
	...
	public function actionDelete($id)
	{
				mysql_connect('localhost','root','HEUganda@2017');
                mysql_select_db('ur2');
                $query = mysql_query("SELECT * FROM `pp_purchaseorddt` WHERE id='$id'") or die(mysql_error());
                $r = mysql_fetch_array($query);
                $pp_purordnum = $r['pp_purordnum'];
                $pp_rowamt = $r['pp_rowamt'];
                $query2 = mysql_query("SELECT * FROM `pp_purchaseordhd` WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
                $row = mysql_fetch_array($query2);
                $pp_discrate = $row['pp_discrate'];
                $pp_amount = $row['pp_amount'] - $pp_rowamt;
                $pp = $pp_amount;
                //echo $pp.'<br>';
                $pp_discamt = $pp_amount * $pp_discrate / 100;
                //echo $pp_discamt.'<br>';
                $pp_netamt = $pp - $pp_discamt;
                //echo $pp_netamt.'<br>';
                //mysql_query("UPDATE pp_purchaseordhd SET pp_discamt='$pp_discamt', pp_netamt='$pp_netamt' WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
        try{
	...
	public function actionCodeQuantityUnit()
        {                     
         if (!empty($_GET['term'])) {
			...
			FROM cm_productmaster WHERE cm_name LIKE :qterm OR cm_code LIKE :qterm';
