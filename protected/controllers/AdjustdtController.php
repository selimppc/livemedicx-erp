...
class AdjustdtController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('admin','delete','create','update', 'GetStockNames', 'GetProdNames'),
				'users'=>array('@'),
			),
  ...
  function actionGetStockNames() //create by selim
  {
      $branchName = $_GET['branchName'];
      //$date = date("Y-m-d");

      if (!empty($_GET['term'])) {
          $sql = "SELECT cm_code as value, CONCAT(cm_name,' -- ' ,available,' -- ', im_unit) as label, im_BatchNumber as batch, im_ExpireDate as expire, im_unit as unit, im_rate as rate, available as available
              FROM im_vw_stock
  ...
  function actionGetProdNames() // created by amit
  {
    //$date = date("Y-m-d");
    if (!empty($_GET['term'])) {
        $sql = "SELECT cm_code as value, cm_name as label, '' as batch, '' as expire, cm_stkunit as unit, cm_costprice as rate
    ...
    /*function actionGetStockNames()
    {
        ...
        //$date = date("Y-m-d");
        echo "Test by amit";
        if($adjustmentType == "Positive Adjustment"){
            if (!empty($_GET['term'])) {
