...
class JournaldtController extends Controller
{
	...
	public function loadModel($id)
	{
		...
		if($model===null)
			throw new CHttpException(404,'You must first save this voucher detail before it can be deleted.');
		return $model;
	}
    ...
    public function actionCreate($am_vouchernumber)
    {
        ...
        mysql_select_db('ur2');
        $voucher = $_GET['am_vouchernumber'];
        //echo $voucher;
         $query = mysql_query("SELECT COUNT(`am_vouchernumber`) AS count FROM `am_voucherdetail` WHERE `am_vouchernumber`='$voucher'");
         $row = mysql_fetch_array($query);
         $voucher_no = $row['count'];
         //echo $voucher_no.'<br>';
        if($voucher_no>1){
            mysql_query("UPDATE am_vouhcerheader SET am_status='Balanced' WHERE am_vouchernumber='$voucher'");
        }
