	...
	protected function performAjaxValidation($model)
	{
		...
		public function actionCreateGRN($id){
			   ...
			   $vGrnNumber = $result['vGrnNumber'];
			  	...
			  	mysql_select_db('ur2');
			  	$sql = mysql_query("SELECT * FROM pp_purchaseordhd WHERE id='$id'");
			  	$row = mysql_fetch_array($sql);
			  	$pp_discamt = $row['pp_discamt'];
			  	$pp_amount = $row['pp_amount'];
			  	$pp_netamt = $pp_amount - $pp_discamt;
			  	mysql_query("UPDATE pp_purchaseordhd SET pp_netamt='$pp_netamt' WHERE id='$id'");	   
		...
		public function actionConfirmGRN($id, $im_grnnumber){
			...
			if ($grndetail > "") {
				   ...
				   mysql_connect('localhost','root','HEDrc@2017');
                mysql_select_db('ur2');
                $sql = mysql_query("SELECT * FROM im_grnheader WHERE id='$id'");
                $row = mysql_fetch_array($sql);
