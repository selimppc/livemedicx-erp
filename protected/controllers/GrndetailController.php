...
class GrndetailController extends Controller
{
		...
		if(isset($_POST['Grndetail']))
		{
            ...
            if($model->validate())
            {
                ...
                if($result ==1){
                    ...
                    mysql_select_db('ur2');
                    $query = mysql_query("SELECT * FROM `im_grnheader` WHERE im_grnnumber='$vGrnNumber'") or die(mysql_error());
                    $row = mysql_fetch_array($query);
                    $im_discrate = $row['im_discrate'];
                    $im_amount = $row['im_amount'];
                    $im_discamt = $im_amount * $im_discrate / 100;
                    $im_netamt = $im_amount - $im_discamt;
                    mysql_query("UPDATE im_grnheader SET im_discamt='$im_discamt', im_netamt='$im_netamt' WHERE im_grnnumber='$vGrnNumber'");
                    $sql = mysql_query("SELECT * FROM `pp_purchaseordhd` WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
                    $ro = mysql_fetch_array($sql);
                    $pp_discamt = $ro['pp_discamt'];
                    $pp_amount = $ro['pp_amount'];
                    $pp_netamt = $pp_amount - $pp_discamt;
                    mysql_query("UPDATE pp_purchaseordhd SET pp_netamt='$pp_netamt' WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
                    }else{
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Grndetail']))
		{
            ...
            mysql_connect('localhost','root','HEDrc@2017');
            mysql_select_db('ur2');
            $query = mysql_query("SELECT * FROM `im_grnheader` WHERE id='$id'") or die(mysql_error());
            $row = mysql_fetch_array($query);
            $im_discrate = $row['im_discrate'];
            $im_amount = $row['im_amount'];
            $im_discamt = $im_amount * $im_discrate / 100;
            $im_netamt = $im_amount - $im_discamt;
            $pp_purordnum = $row['pp_purordnum'];
            mysql_query("UPDATE im_grnheader SET im_discamt='$im_discamt', im_netamt='$im_netamt' WHERE id='$id'");
            $sql = mysql_query("SELECT * FROM `pp_purchaseordhd` WHERE pp_purordnum='$pp_purordnum'") or die(mysql_error());
            $ro = mysql_fetch_array($sql);
            $pp_discamt = $ro['pp_discamt'];
            $pp_amount = $ro['pp_amount'];
	...
	public function actionDelete($id)
	{
			mysql_connect('localhost','root','HEDrc@2017');
            mysql_select_db('ur2');
            $qu = mysql_query("SELECT * FROM `im_grndetail` WHERE id='$id'") or die(mysql_error());
            $r = mysql_fetch_array($qu);
            $im_grnnumber = $r['im_grnnumber'];
		...
		if(!isset($_GET['ajax']))
			...
			$query = mysql_query("SELECT * FROM `im_grnheader` WHERE im_grnnumber='$im_grnnumber'") or die(mysql_error());
            $row = mysql_fetch_array($query);
            $im_discrate = $row['im_discrate'];
            $im_amount = $row['im_amount'];
            $im_discamt = $im_amount * $im_discrate / 100;
            $im_netamt = $im_amount - $im_discamt;	
            $im_purordnum = $row['im_purordnum'];
            $sql = mysql_query("SELECT * FROM `pp_purchaseordhd` WHERE pp_purordnum='$im_purordnum'") or die(mysql_error());
            $ro = mysql_fetch_array($sql);
            $pp_discamt = $ro['pp_discamt'];
            $pp_amount = $ro['pp_amount'];
            $pp_netamt = $pp_amount - $pp_discamt;
            mysql_query("UPDATE pp_purchaseordhd SET pp_netamt='$pp_netamt' WHERE pp_purordnum='$im_purordnum'") or die(mysql_error());
			mysql_query("UPDATE im_grnheader SET im_discamt='$im_discamt', im_netamt='$im_netamt' WHERE im_grnnumber='$im_grnnumber'");
