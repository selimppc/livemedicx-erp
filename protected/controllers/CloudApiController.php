...
class CloudApiController extends CController
{
	...
	*/
	...
	public function actionFetchJsonData(){
		...
		$area = 5;   /* area : 1=NL, 2=Burundi,3=Rwanda,4=DRC,5=Uganda,6=Haiti */
		...
		if(count($result)>0){
			foreach($result as $hd){
				...
				if( count($record)>0 ){
					// Do Nothing ...
					echo ("Order already Synchronised !"),'                                                                                       ','Invoice No: ',' ',$this->actionInvoiceNo(),'                                                                                      ';
					//return true;
					...
					if($customer_code){
						...
						$model->sm_storeid = $customer_code['cm_branch'];
						...
						try {
							...
							//echo  ("Mismatched Product Code from Cloud to ERP. "),'    ',;
							//echo $hd['sm_refe_code'],$hd['cm_cuscode'];
							//echo insertuser = "onTab";
							
							echo  "Product Code did not matched, TAB-Cloud to ERP",'                                    ','Invoice No: ','      ',$this->actionInvoiceNo(),'                                                                     ','Customer ID: ','   ', $hd['cm_cuscode'],'                                                                                      ','Product Code: ','  ',$dt['cm_code'],'                                                                                                    ','Date of Order: ',' ',$hd['sm_date'],'                                                                                                       ','Products Qty: ','  ',$dt['sm_unit_qty'], '                                                                                            ';
						...
						//echo "Gerant Id missing!=>";
   								echo '                                        ',"S-T-A-T-U-S",'                                                   ';
  								echo  "Gerant Id missing!",'                                                                                         ',                                                'Customer ID: ',' ', $hd['cm_cuscode'],'                                                                                          ',                                                        'Order Qty: ','     ', count($hd['cm_cuscode'])                                                                              ,'                                                                                               ',                                                                'Date of Order:',' ',$hd['sm_date'],'                                                                                                               ';
