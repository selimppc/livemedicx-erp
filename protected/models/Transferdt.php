...
class Transferdt extends CActiveRecord
{
	...
	public function cmCode($cm_code)
	{
	 ...
	 public function greaterThanZero($im_quantity)
    {

        if ($this->$im_quantity<=0)
            $this->addError($im_quantity, 'Quantity should not be blank or zero');
	...
	public function search($im_transfernum)
	{
		...
		$criteria->compare('im_quantity',$this->im_quantity,true);
