...
if(isset($_POST["pdf"])){
  ...
  //$pdf->Cell(80);
  $pdf->Cell(0,10,'Reverse Entry',0,0,'L');
  //$pdf->Ln(10);
  ...
  while($row = mysql_fetch_array($sql)){
    ...
    $pdf->Cell(39, $row_height, 'Period', 0, 0, 'R');
    ...
    $pdf->Cell(122, $row_height, $row['am_note'], 0, 0, 'R');
    $pdf->Ln(7);
    $pdf->Cell(25, $row_height, 'Code', 1, 0, 'L', 1);
    $pdf->Cell(75, $row_height, 'Accounts Description', 1, 0, 'L', 1);
    $pdf->Cell(15, $row_height, 'Currency', 1, 0, 'C', 1);
    $pdf->Cell(15, $row_height, 'Ex. Rate', 1, 0, 'C', 1);
    $pdf->Cell(35, $row_height, 'Debit', 1, 0, 'L', 1);
    $pdf->Cell(35, $row_height, 'Credit', 1, 0, 'L', 1);
    $pdf->Cell(35, $row_height, 'Debit Local', 1, 0, 'L', 1);
    $pdf->Cell(35, $row_height, 'Credit Local', 1, 0, 'L', 1);
  ...
  while($product_row = mysql_fetch_array($product)){
    ...
    $pdf->Cell(25, 15, $product_row['am_accountcode'], 0, 0, 'L');
    $pdf->Cell(75, 15, $product_row['am_description'], 0, 0, 'L');
    $pdf->Cell(15, 15, $product_row['am_currency'], 0, 0, 'L');
    $pdf->Cell(15, 15, $product_row['am_exchagerate'], 0, 0, 'L');
    $pdf->Cell(35, 15, $debit, 0, 0, 'L');
    $pdf->Cell(35, 15, $fcredit, 0, 0, 'L');
    $pdf->Cell(35, 15, $debitl, 0, 0, 'L');
    $pdf->Cell(35, 15, $fcreditl, 0, 0, 'L');
    $pdf->Ln(5);
  }
  $pdf->Ln(5);
  ...
  mysql_query("INSERT INTO `jurnal_sum` VALUES('','$Dabets','$fCredits','$Dabetls','$fCreditls')");
    ...
    $pdf->Cell(15, 15, '', 0, 0, 'C');
    $pdf->Cell(35, 15, $Dabet, 0, 0, 'C');
    $pdf->Cell(35, 15, $fCredit, 0, 0, 'C');
    $pdf->Cell(35, 15, $Dabetl, 0, 0, 'C');
    $pdf->Cell(35, 15, $fCreditl, 0, 0, 'C');
    ...
    $pdf->Cell(75, 15, '', 0, 0, 'L');
    $pdf->Cell(15, 15, '', 0, 0, 'L');
    $pdf->Cell(15, 15, 'Total Local Report Summary:', 0, 0, 'R');
    $pdf->Cell(35, 15, '', 0, 0, 'L');
    $pdf->Cell(35, 15, '', 0, 0, 'L');
    $pdf->Cell(35, 15, number_format($debetl_sum_row['dabetl_sum'],2), 0, 0, 'L');
    $pdf->Cell(35, 15, number_format($creditl_sum_row['creditl_sum'],2), 0, 0, 'L');
  $pdf->Output();
  ...
  if(mysql_num_rows($sql) > 0)  
    {  
      ...
      <table class="table">  
        <tr>  
          <th colspan="8" align="left"><h2><u>Reverse Entry</u></h2></th>    
        </tr> 
        <tr>  
    ...
    header("Content-Disposition: attachment; filename=Reverse_Entry.xls");  
    echo $output;  
