...
if(isset($_POST["pdf"])){
  ...
  $pdf->Cell(0,0,'Profit & Loss Statement',0,0,'C');
  $pdf->Ln(10);
  ...
  $pdf->SetX(10);
    if($word_four=='Summary'){
      ...
      $pdf->Ln(15);
      $pdf->Cell(135, $row_height, 'Total Expenses', 0, 0, 'R', 1);
  ...
  if($word_four=='Summary'){
    ...
    if(mysql_num_rows($sql) > 0){
        ...
        <table  class="table table-bordered" width="900">
          <tr>
            <th colspan="4" class="text-center"><h2>Profit & Loss Statement <br>As at  '.$word_two.' '.$word_one.'</h2></th>
          </tr>
          <tr>
    ...
    header("Content-Disposition: attachment; filename=Profit Loss.xls");  
    echo $output;  
