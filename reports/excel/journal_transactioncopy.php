...
$output = ''; 
$sq = mysql_query("SELECT * FROM `am_vouhcerheader` WHERE am_date BETWEEN '$from_date' AND '$to_date'");
while($r = mysql_fetch_array($sq)){
      $voucher = $r['am_vouchernumber'];
      $am_voucher  = substr($voucher,4,8);
      $am_vouchernumber = $first_word.$am_voucher;
      $sql ='';
      if($first_word==false && $second_word==false && $third_word==false){
        ...
        $sql = mysql_query("SELECT * FROM `am_vouhcerheader` WHERE am_vouchernumber ='$am_vouchernumber' AND am_branch ='$second_word' AND am_status ='$third_word' AND am_date BETWEEN '$from_date' AND '$to_date'") or die(mysql_error());
      }elseif($first_word==true && $second_word==false && $third_word==false){
        $sql = mysql_query("SELECT * FROM `am_vouhcerheader` WHERE am_vouchernumber ='$am_vouchernumber' AND am_date BETWEEN '$from_date' AND '$to_date'") or die(mysql_error());
      }elseif($first_word==true && $second_word==false && $third_word==true){
        $sql = mysql_query("SELECT * FROM `am_vouhcerheader` WHERE am_vouchernumber ='$am_vouchernumber' AND am_status ='$third_word' AND am_date BETWEEN '$from_date' AND '$to_date'") or die(mysql_error());
        ...
        $sql = mysql_query("SELECT * FROM `am_vouhcerheader` WHERE am_vouchernumber ='$am_vouchernumber'AND am_branch ='$second_word' AND am_date BETWEEN '$from_date' AND '$to_date'") or die(mysql_error());
      }elseif($first_word==false && $second_word==false && $third_word==true){
        $sql = mysql_query("SELECT * FROM `am_vouhcerheader` WHERE am_status ='$third_word' AND am_date BETWEEN '$from_date' AND '$to_date'") or die(mysql_error());
      }
    $row = mysql_fetch_array($sql);
    $vouchernumber = $row['am_vouchernumber'];
    if($vouchernumber==false){
      ...
      <table class="table">  
        <tr>  
          <th colspan="8" align="left"><h2><u>Journal Transaction</u></h2></th>    
        </tr> 
        <tr>  
        ...
        <tr>
          ...
          <td align="left">'.$row['am_period'].'</td>
          <td></td>
          <td></td>
        ...
        $output .= '    
            <tr>
              <th colspan="4" class="text-right">Report Summary</th>
...
//     $pdf->Cell(56, $row_height, $row['am_date'], 0, 0, 'R');
//     $pdf->Cell(37, $row_height, 'Period', 0, 0, 'R');
//     $pdf->Cell(13, $row_height, $row['am_period'], 0, 0, 'R');
//     $pdf->Cell(120, $row_height, 'Status', 0, 0, 'R');
//     $pdf->Cell(20, $row_height, $row['am_status'], 0, 0, 'R');
//     $pdf->Ln(7);
//     $pdf->Cell(10, $row_height, 'Branch', 0, 0, 'L');
//     $pdf->Cell(50, $row_height, $row['am_branch'], 0, 0, 'R');
//     $pdf->Cell(40, $row_height, 'Note', 0, 0, 'R');
//     $pdf->Cell(19, $row_height, $row['am_note'], 0, 0, 'R');
//     $pdf->Ln(7);
...
//     $pdf->Cell(85, 15, '', 0, 0, 'C');
//     $pdf->Cell(25, 15, '', 0, 0, 'C');
//     $pdf->Cell(25, 15, 'Report Summary', 0, 0, 'C');
//     $pdf->Cell(25, 15, number_format($debet_sum_row['dabet_sum'],2), 0, 0, 'C');
//     $pdf->Cell(25, 15, number_format($credit_sum_row['credit_sum'],2), 0, 0, 'C');
