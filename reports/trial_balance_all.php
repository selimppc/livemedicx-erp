  	...
  	<div class="container">
  		<div class="row">
              ...
              ?>
                  ...
                  <h4 style="border-bottom:1px solid #CCC"><b>From Date <?php echo $from_date?> To <?php echo $to_date?></b></h4>
  							</div>
                <div class="col-md-2"></div>
  						</div>
                <form method="post" action="excel/trial_balance_all.php?f_word=<?php echo $f_word?>&from_date=<?php echo $from_date?>&to_date=<?php echo $to_date?>" >
                        ...
                        while($con_row = mysql_fetch_array($content)){
                          ...
                          print'
                              <tr>
                                ...
                                <td>'.number_format($sumbfC,2).'</td>
                                <td>'.number_format($sumbfD,2).'</td>
                                <td>'.number_format($sumcdrC,2).'</td>
                                <td>'.number_format($sumcdrD,2).'</td>
                                <td>'.number_format($sumcfC,2).'</td>
                                <td>'.number_format($sumcfD,2).'</td>
                              </tr>
                              ';
                              }
                  ...
                  <tr>
                    ...
                    while($row = mysql_fetch_array($sql)){
                      ...
                      while($sub_row = mysql_fetch_array($sub_sql)){
                        ...
                        while($con_row = mysql_fetch_array($content)){
                          $am_accountcode = $con_row['am_accountcode'];
                          $sum_bf = mysql_query("SELECT SUM(c_baseamt)AS sumbf FROM am_balance WHERE c_accountcode='$am_accountcode' AND c_date <'$from_date'") or die(mysql_error());
                          $sum_bf_row = mysql_fetch_array($sum_bf);
                          $sumbf = $sum_bf_row['sumbf'];
                          $sumbfC = '';
                          $sumbfD = '';
                          ...
                          $sum_cdr = mysql_query("SELECT SUM(c_baseamt)AS sumcdr FROM am_balance WHERE c_accountcode='$am_accountcode' AND c_date BETWEEN '$from_date' and '$to_date'") or die(mysql_error());
                          $sum_cdr_row = mysql_fetch_array($sum_cdr);
                          $sumcdr = $sum_cdr_row['sumcdr'];
                          $sumcdrC = '';
                          $sumcdrD = '';
                          ...
                          $sum_cf = mysql_query("SELECT SUM(c_baseamt)AS sumcf FROM am_balance WHERE c_accountcode='$am_accountcode' AND c_date BETWEEN '$from_date' and '$to_date'") or die(mysql_error());
                          $sum_cf_row = mysql_fetch_array($sum_cf);
                          $sumcf = $sum_cf_row['sumcf'];
                          $sumcfC = '';
                          $sumcfD = '';
                          ...
                          print'
                              <tr>
                                <td>'.$con_row['am_accountcode'].'&nbsp;'.$con_row['am_description'].'</td>
                                <td>'.number_format($sumbfC).'</td>
                                <td>'.number_format($sumbfD).'</td>
                                <td>'.number_format($sumcdrC,2).'</td>
                                <td>'.number_format($sumcdrD,2).'</td>
                                <td>'.number_format($sumcfC).'</td>
                                <td>'.number_format($sumcfD).'</td>
                              </tr>
                              ';
                              }
                        }
