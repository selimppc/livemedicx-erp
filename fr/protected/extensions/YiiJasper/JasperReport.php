...
class JasperReport extends CComponent {
    ...
    */
    public function parseHtml($html){
        ...
        $html = preg_replace_callback('/(<img.+src\s*=\s*")\/jasperserver([^"]*)(".*?>)/', function($im){
            $this->req->get($im[2]);
            $this->req->send();           
            $imagen  = base64_encode($this->req->getResponse());
            $mimeType= $this->req->getHeader('Content-Type');
